...
float colorValues[width][height][3]; 

static void *
renderImage( void* arg ){
    Arg_t* obj;
    obj=(Arg_t*)arg; //cast argument
    int id=obj->id;
    int numThreads=obj->numThreads;
    scene_t scene=*(obj->scene);

//TODO: Figureout how to divide up work
    Vec3 camera_pos;
    ...
    /* for every pixel */
    for( int px=id; px<width; px+=numThreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            for(int i=0; i<3; i++){
	    	colorValues[px][py][i]=scaled_color[i];
	    }
...
main( int argc, char **argv )
{
    ...
    Arg_t arguments[nthreads]; 

    //Initialize arguments
    for(int i=0; i<nthreads; i++){
      arguments[i].id=i; 
      arguments[i].numThreads=nthreads; 
      arguments[i].scene=&scene; 
    }
    
    for(int i=0; i<nthreads; i++){
      int ret=pthread_create(&(threads[i]), NULL, renderImage, &(arguments[i]));
      if(ret)
	{
    ...
    for(int i=0; i<nthreads; i++){
      ...
      if(ret){
	fprintf(stderr, "Error when waiting for thread termination. \n");
	exit(1); 
      }
	...
	for(int j=0; j<height; j++){
		printf( "%.0f %.0f %.0f\n",
		colorValues[i][j][0], colorValues[i][j][1], colorValues[i][j][2] );	
	}
	printf("\n");
